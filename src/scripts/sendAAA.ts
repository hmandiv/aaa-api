...
export async function sendAAA(receiverAddress: string) {
  try {
    ...
    const txn = algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
      ...
      to: receiverAddress,
      amount: amount,
      assetIndex: AAA_ASA_ID,
      note: new Uint8Array(Buffer.from("AAA App: 0.01 AAA sent")),
      suggestedParams,
    });
...
(async () => {
  ...
  const result = await sendAAA(receiver);
  console.log(result);
})();
